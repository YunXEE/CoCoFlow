...
- `NetworkCameraRigBinder`: 将网络层的本地相机权威翻译成 `CameraDirector.BindLocalRig/ClearLocalRig`。它不依赖 Fusion；Fusion adapter 只需要在 spawn 或 authority 变化时调用 `SetLocalCameraAuthority(...)`。
...
flowchart TD
  ...
  LocalAuthority["Local camera authority"] --> CameraBinder["NetworkCameraRigBinder"]
...
- Camera 是本地表现层。不要把 camera mode 同步成 `CharacterContext`；只在拥有或观察目标的客户端绑定本地 camera rig。
