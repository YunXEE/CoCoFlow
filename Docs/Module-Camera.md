...
CutsceneAnchor / SpectateAnchor / BossCameraAnchor
  CameraRig
  VCam_Cutscene 或 VCam_BossCombat
```
...
- winner 的 `CurrentCamera.Priority` 会被设为 rig priority，其他已注册 rig 的已配置相机会被清到 `0`。
...
`Aim` 只是项目约定的 mode id，不是框架内置枚举。它通常保留在玩家自己的 `CameraRig` 内部，因为第三人称瞄准是玩家表现选择，不应该变成 Director 的全局 request。

推荐结构是玩家 prefab 内部有一个可由输入旋转的 `AimCore`：

- `Explore` 这类自由移动 mode 下，`AimCore` 可以跟随输入旋转，但角色 prefab 不一定跟着转。
- `Aim` 这类瞄准 mode 下，`AimCore` 继续控制镜头朝向，`CameraAimCoupler.Coupled` 可以把 AimCore 的旋转同步给绑定 Transform。绑定 Root/父级这类祖先目标时，Coupler 只把水平 yaw 搬到 Root，并修正 AimCore 本地 yaw；绑定非祖先目标时才写完整 world rotation。
