...
{
    public class PersistenceLoadEditor : EditorWindow
    {
        // 在 Unity 顶部菜单栏创建入口
        ...
        {
            // 弹出一个小窗口
            GetWindow<PersistenceLoadEditor>("存档加载编辑器");
        }
        ...
        private void OnGUI()
        {
            ...
            // 1. 配置槽位上限
            PersistenceSaveLoadSystem.MaxSaveSlots = EditorGUILayout.IntField("支持的最大槽位数量", PersistenceSaveLoadSystem.MaxSaveSlots);

            // 2. 指定下次读盘的槽位 (使用 EditorPrefs 保证你关了 Unity 明天再开，设置还在)
            int currentDebugSlot = EditorPrefs.GetInt("CoCo_DebugSaveSlot", 0);
            int newDebugSlot = EditorGUILayout.IntSlider("当前测试槽位", currentDebugSlot, 0, PersistenceSaveLoadSystem.MaxSaveSlots - 1);
            
            if (newDebugSlot != currentDebugSlot)
            {
                EditorPrefs.SetInt("CoCo_DebugSaveSlot", newDebugSlot);
                PersistenceSaveLoadSystem.CurrentSlotIndex = newDebugSlot; // 同步给运行时系统
            }
