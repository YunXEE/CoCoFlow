<!-- CoCoFlow 0.4 冻结契约清单。方案与 PR 按所属模块引用对应节核对，非全量。 -->

## Pre3~Pre6 StateFlow / Runtime / Commit / Temporal 核心契约
...
- [ ] This PR targets `dev/0.4.0`, never `master` directly.
...
### 集成验证（Unity 6 clean host）

- 宿主工程 / 版本 / 包引用 / 受测 Final Head：
...
- [ ] Test result files, allocation/performance evidence, build logs, or
      screenshots are linked in the PR Evidence section.
...
- [ ] Test result files, allocation/performance evidence, build logs are linked
      in the PR Evidence section.
...
- [ ] Unity Package Validation Suite passed without adding an unexplained
      exception, and package/version metadata matches `package.json`.
